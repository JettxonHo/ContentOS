...
1. M2 — Source and Workflow Foundation — is in progress. `M2-FETCH-001`, `M2-SRC-004`, `M2-WF-004A`, and `M2-WF-004B` are completed. `M2-WEB-001A` is in review: the active Workspace composes authoritative Source and URL-intake REST reads with the existing notification/polling recovery. Source editing, Version/Approval review UI, and M2 acceptance work remain not started.
