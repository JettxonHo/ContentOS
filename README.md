...
1. M2 — Source and Workflow Foundation — is in progress. `M2-FETCH-001` and `M2-SRC-004` are completed. `M2-WF-004A` is In Review with read-only Workflow projection and Timeline REST queries. The internal current-Approved Source input projection does not provide Research readiness, Frozen Input, API/UI, or Research execution. SSE, Source workspace, and M2 acceptance work remain not started.
