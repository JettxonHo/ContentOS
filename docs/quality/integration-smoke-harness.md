...
| `corepack pnpm test:integration:concurrent:fg07` | **Yes**         | Two complete isolated runs limited to the fixed Fetcher Gateway `[FG-07A]` / `[FG-07B]` diagnostic pair. |
...
After both children succeed and the coordinator verifies isolation and final owned cleanup, the command emits exactly one fixed, LF-terminated success record:

```text
Harness concurrent final coordinator=verified children=2 isolation=verified cleanup=verified
```

The record is written only after the coordinator resolves and is not written when coordination or final cleanup rejects. It contains no child output, process or container identity, paths, credentials, timestamps, or other dynamic fields.

Concurrent failure output keeps two cleanup axes independent. A child Harness teardown record is parsed only from a complete newline-terminated ANSI-stripped line in one of the fixed `contentos smoke harness teardown failed: cleanup=<category> physical=<state> capsule=<state>` or `contentos smoke setup failed: setup=<category> teardown=failed cleanup=<category> physical=<state> capsule=<state>` forms. One allowlisted child category (`managed-process`, `process-control`, `object-storage`, `compose`, `compose-verify`, `root`, or `synthetic`) is emitted as `category=cleanup-<category> child-physical=<clean|incomplete> child-capsule=<removed|preserved>`. Missing, malformed, unknown, multi-category, conflicting, or incomplete Harness evidence emits only `category=cleanup-unclassified`; an ordinary `teardown=failed` or `error during close` without a complete Harness record retains generic `cleanup-failed`. A final partial capture segment is ignored, and raw child output is never emitted.

The Integration global teardown catch preserves the non-zero exit status, formats only the internal structured `HarnessCleanupError`, writes exactly one LF-terminated fixed record to the existing child stderr pipe, and rethrows the same error object. A malformed or unknown error emits only the constant `cleanup=unclassified physical=incomplete capsule=preserved` record. No cleanup attempt, wait, drain, retry, signal, or raw error transport is added; Vitest may print an identical record later and the bounded parser deduplicates it.

The parent `owned-cleanup` field reflects only exact claim-authorized verification: success is `owned-cleanup=verified`, while one deterministic failure axis is `owned-cleanup=failed-ownership`, `failed-process`, `failed-compose`, `failed-verification`, or `failed-root`. Invalid or missing claims/control records map to `ownership`; managed-process stop failures to `process`; Compose down/input failures or claimed Compose residue to `compose`; Compose project listing/parsing failures to `verification`; and child/parent run-root removal or residue to `root`. Repeated failures in one axis are deduplicated; multiple axes or an unexpected error map to `failed-unclassified`. Child teardown never overwrites a verified parent result. The same parent mapping is used after a child error, after successful children, and when parent build failure is cleaned before child start.
...
Only when that safe test field is exactly `fetcher-gateway-api.test.ts` may the coordinator append `case=fg-01` through `case=fg-06`, `case=fg-07a`, `case=fg-07b`, or `case=fg-08` through `case=fg-11`. Every complete, newline-terminated Vitest `FAIL` metadata line anchored to the exact Fetcher Gateway integration path must contain exactly one allowlisted `[FG-01]`–`[FG-06]`, `[FG-07A]`, `[FG-07B]`, or `[FG-08]`–`[FG-11]` marker. Repeated lines carrying the same marker are deduplicated; a missing marker on any matching line, incomplete or unknown metadata, or multiple different markers produces `case=unclassified`. Markers on adjacent assertions, stacks, or other non-metadata lines are ignored and cannot be attributed. Other failure categories and other test basenames retain their existing diagnostic shape. Raw output, test titles, assertions, stacks, URLs, credentials, paths, PIDs, and logs are never emitted or persisted.

The fixed `corepack pnpm test:integration:concurrent:fg07` command is the only focused diagnostic mode. It is equivalent to the exact `--focus fg-07` tuple and starts two complete Integration harnesses through the same ownership and cleanup coordinator, selecting only `fetcher-gateway-api.test.ts` and the `[FG-07A]` / `[FG-07B]` titles. Unsupported, extra, or arbitrary CLI arguments fail before claims, builds, temporary roots, or child processes are created. The ordinary concurrent command remains unchanged.
