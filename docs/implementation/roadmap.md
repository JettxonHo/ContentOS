...
| `M2-QUAL-005 — Safe Fetcher Gateway Case Attribution` | Refine the attributed Fetcher Gateway test-file failure to one static non-sensitive case ID without exposing raw output or changing test behavior. | M2-QUAL-004 completed; M2-QUAL-003 blocked at `fetcher-gateway-api.test.ts` | **Ready.** [Work Packet](work-packets/m2-qual-005-safe-fetcher-gateway-case-attribution.md), Issue #153. Diagnostic-only static case IDs and bounded parser evidence; no guessed repair, timeout/retry change, product behavior, or M3 work. |
