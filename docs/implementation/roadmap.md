...
| `M2-QUAL-003 — Worker Dispatcher Reconciliation Observation Stability` | Wait for the authoritative Outbox acknowledgement after a repaired Redis Job appears, without changing production behavior. | PR #146 final-head Integration smoke failure | **Blocked pending M2-QUAL-004.** [Work Packet](work-packets/m2-qual-003-worker-dispatcher-observation-stability.md), Issue #147. The bounded repaired-Job synchronization implementation is preserved, but repeated Concurrent validation still reports an unclassified child test failure. Safe failure attribution is required before exact remediation and completion. M2 remains In Progress and M3 remains Not Started. |

| `M2-QUAL-004 — Safe Concurrent Failure Attribution` | Attribute a concurrent child test failure to one safe Integration test basename without exposing raw output or sensitive values. | M2-QUAL-003 blocked by unclassified concurrent failure | **In Review — independent review passed; awaiting GitHub CI.** [Work Packet](work-packets/m2-qual-004-safe-concurrent-failure-attribution.md), Issue #149. The bounded parser and diagnostic tests are implemented. Test-harness diagnostics only; no raw output persistence, timeout/retry change, production, dependency, Schema, migration, Compose, CI, DEC, Acceptance Record, or M3 behavior change. |
...
| `M2-GOV-006 — M2 Exit Review and Acceptance Record 002` | Re-run the complete M2 exit evidence on one exact post-remediation `main` commit and publish a new immutable Passed or Blocked record. | `M2-MAINT-002` completed; Record 001 retained | **Blocked pending M2-QUAL-003 and M2-QUAL-004.** [Work Packet](work-packets/m2-gov-006-m2-exit-review-002.md), Issue #144. Closed Draft PR #146 exposed a real integration-test observation race before publication; repeated Concurrent validation remains safely unclassified. No Acceptance Record 002 was merged. After the diagnostic and exact remediation complete, the exit review must restart from the new exact `main` commit. M2 remains In Progress and M3 remains Not Started. |
