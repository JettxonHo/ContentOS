...
| `M2-QUAL-011 — Safe Focused Integration Setup Failure Attribution and Replay` | Attribute only the final generic Integration setup fallback to one fixed safe phase, repair the teardown-independence evidence gap, then replay the exact focused Worker command under a bounded first-red rule. | M2-QUAL-003 blocked at `setup=setup-failed teardown=clean` | **Ready for renewed implementation.** [Work Packet](work-packets/m2-qual-011-safe-focused-integration-setup-attribution-replay.md), Issue #175 remains Open. The historical **Blocked — Browser setup gate failure** record and independent Blocked-evidence reviewers remain retained as history; the current renewed contract is Ready, but no implementation code or replay PASS is claimed. Renewed planning base is `ba99ef750141e2eef83e54742bc816c65d4bca0b`, after M2-QUAL-012 completed through PR #180 and status PR #181, so Browser safe setup transport is available. The preserved six-file implementation diff is read-only fact reference; its unit only awaited `Promise.resolve()` and did not execute teardown/state removal, so the renewed implementation must replace that test through the narrow snapshot → await teardown → classification helper seam. Renewed Definition-of-Ready correctness and governance reviews both PASS (`/root/m2_qual_011_renewed_dor_correctness`, `/root/m2_qual_011_renewed_dor_governance`; role `DEFINITION_OF_READY_REVIEWER`, requested `gpt-5.6-sol` High, runtime `UNVERIFIED_RUNTIME_MODEL`); terminal/publication findings are closed, with no Blocking Design Question or new DEC. The historical Worker replay count is `0`; the focused Harness/concurrent tests, root `check`, full Integration, and one ordinary Browser acceptance gate must all be green before renewed Worker replay begins. Browser red/missing/residue stops before Worker with no rerun-to-green. M2-QUAL-003 and M2-GOV-006 remain Blocked, M2 remains In Progress, M3 remains Not Started. |
