...
**Current status: In Progress.** `M0-ENG-001 — Workspace and TypeScript Baseline`, `M0-ENG-002 — Application Skeletons`, `M0-ENG-003 — Web Skeleton Loopback Binding`, `M0-INFRA-001 — Local State Services`, `M0-QUAL-001 — Local Quality Toolchain`, and `M0-QUAL-002 — Integration Smoke Harness` are merged. `M0-CI-001 — CI Skeleton` is implemented and in review: its bounded GitHub Actions workflow runs the Docker-independent quality gate, repository-integrity checks (Markdown links, Canonical Decision register, Secret scan), and the Docker-dependent integration smoke harness; applications remain disconnected through product code, and schema, migrations, Queue behavior, product tests, and a full release gate remain absent.
...
| `M0-CI-001 — CI Skeleton`                        | Run the M0 command suite and basic secret/documentation checks in CI.                                          | `M0-QUAL-001`, `M0-QUAL-002`                | **In Review.** No release platform, production deployment, or full release gate.           |
