...
**Current status: In Progress.** `M2-SRC-001 — Pasted-text Source Capture and Approval Foundation` is completed (PR #40, `1a50d2d`), as is `M2-SRC-002 — .md/.txt File-upload Source Capture and Upload Quarantine` (PR #42, `3f418bf`), `M2-WF-001 — Template, Instance, Node, and Event Persistence Foundation` (PR #55, `8ec67ae`), and `M2-WF-002 — Atomic URL-capture Command, Task, and Transactional Outbox` (PR #60, `3928b55`). M2-WF-002 adds only the protected durable Command boundary; Queue delivery, Fetcher execution, Source evidence, and later M2 Work Items remain unavailable; M3 remains not started.
...
| `M2-WF-003A — Transactional Outbox Dispatcher`                           | Deliver the existing minimum Fetcher Task envelope from PostgreSQL Outbox state to BullMQ with current-generation delivery recovery. | `M2-WF-002`, `M2-DES-005` accepted | **In Review.** Queue delivery only: Worker Dispatcher, BullMQ transport, and Outbox-ledger recovery. No Fetcher consumer, service identity, claim/lease, result, Source evidence, public URL retrieval, UI/SSE, Agent, or M3 behavior.                                     |
