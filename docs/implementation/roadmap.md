...
**Current status: In Progress.** `M2-SRC-001 — Pasted-text Source Capture and Approval Foundation` is completed (PR #40, `1a50d2d`), as is `M2-SRC-002 — .md/.txt File-upload Source Capture and Upload Quarantine` (PR #42, `3f418bf`) and `M2-WF-001 — Template, Instance, Node, and Event Persistence Foundation` (PR #55, `8ec67ae`). `M2-WF-002 — Atomic URL-capture Command, Task, and Transactional Outbox` is implemented and **In Review** pending independent review. The Work Item adds only the protected durable Command boundary; Queue delivery, Fetcher execution, Source evidence, and later M2 Work Items remain unavailable; M3 remains not started.
...
| `M2-WF-002 — Atomic URL-capture Command, Task, and Transactional Outbox` | Atomically accept one owner URL-capture request and create the lazy initial Workflow primitives, Task, Outbox, and Event. | `M2-WF-001` completed | **In Review.** Core, Contracts, additive `0005`, atomic repository, protected API, rollback/concurrency/migration tests, and OpenAPI are implemented. No Dispatcher, BullMQ execution, Fetcher, Source evidence, Source Approval, UI, SSE, Agent, or M3 behavior. |
