...
| `M2-QUAL-011 — Safe Focused Integration Setup Failure Attribution and Replay` | Attribute only the final generic Integration setup fallback to one fixed safe phase, then replay the exact focused Worker command under a bounded first-red rule. | M2-QUAL-003 blocked at `setup=setup-failed teardown=clean` | **Ready.** [Work Packet](work-packets/m2-qual-011-safe-focused-integration-setup-attribution-replay.md), Issue #175. Independent correctness/executability and governance/scope/security Definition of Ready reviews passed. Scope is limited to in-memory setup-phase attribution, deterministic compatibility/redaction evidence, and at most three focused replay attempts. It does not publish the preserved Worker delta, infer or repair a root cause, change cleanup/retry/timeout behavior, complete M2-QUAL-003 or M2-GOV-006, complete M2, or start M3. |
