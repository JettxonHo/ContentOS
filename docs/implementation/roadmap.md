...
**Current status: Blocked at Exit Review.** All seven planned M0-B engineering Work Items are merged, including `M0-CI-001 — CI Skeleton`. The first [M0-B Exit Review](m0-b-exit-review-001.md) confirms that the M0-B-specific deliverables and local/remote quality checks pass, but the official dependency audit still reports unresolved High-severity advisories. These findings are tracked separately by GitHub Issue #12 and are a Blocking Defect under the common Exit Criteria. M0-B remains blocked until bounded remediation is complete and a new immutable Exit Review passes; M0-C has not started.
