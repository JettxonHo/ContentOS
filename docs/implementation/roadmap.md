...
**Current status: In Progress.** `M2-SRC-001 — Pasted-text Source Capture and Approval Foundation` is completed (PR #40, `1a50d2d`), as is `M2-SRC-002 — .md/.txt File-upload Source Capture and Upload Quarantine` (PR #42, `3f418bf`). `M2-WF-001 — Template, Instance, Node, and Event Persistence Foundation` is In Review. Its fixed catalog and neutral persistence primitives are present, but Workflow execution, Source URL capture, and later M2 Work Items remain unavailable; M3 remains not started.
