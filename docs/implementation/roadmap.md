...
| `M2-QUAL-004 — Safe Concurrent Failure Attribution` | Attribute a concurrent child test failure to one safe Integration test basename without exposing raw output or sensitive values. | M2-QUAL-003 blocked by unclassified concurrent failure | **Ready.** [Work Packet](work-packets/m2-qual-004-safe-concurrent-failure-attribution.md), Issue #149. Two independent Definition of Ready reviews passed. Test-harness diagnostics only; no raw output persistence, timeout/retry change, production, dependency, Schema, migration, Compose, CI, DEC, Acceptance Record, or M3 behavior change. |
