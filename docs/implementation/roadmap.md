...
| `M2-WF-003A — Transactional Outbox Dispatcher`                           | Deliver the existing minimum Fetcher Task envelope from PostgreSQL Outbox state to BullMQ with current-generation delivery recovery. | `M2-WF-002`, `M2-DES-005` accepted | **Ready.** Queue delivery only: Worker Dispatcher, BullMQ transport, and Outbox-ledger recovery. No Fetcher consumer, service identity, claim/lease, result, Source evidence, public URL retrieval, UI/SSE, Agent, or M3 behavior.                                         |
