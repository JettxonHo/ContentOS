...
**Current status: In Progress.** `M2-SRC-001 — Pasted-text Source Capture and Approval Foundation` is completed (PR #40, `1a50d2d`), as is `M2-SRC-002 — .md/.txt File-upload Source Capture and Upload Quarantine` (PR #42, `3f418bf`), `M2-WF-001 — Template, Instance, Node, and Event Persistence Foundation` (PR #55, `8ec67ae`), and `M2-WF-002 — Atomic URL-capture Command, Task, and Transactional Outbox` (PR #60, `3928b55`). `M2-WF-003A — Transactional Outbox Dispatcher` is completed through PR #69, squash-merged as `3211c29ef8e6a934e6473a4f92caf36d8593abc3`; `M2-WF-003B — Fetcher Gateway Claim and Bounded Lease` is completed through PR #73, merged as `c9c92b70a0ccd99be944107120f03dd3a1776da3` (`feat: add fetcher gateway claim lease (#73)`). `M2-WF-002` adds only the protected durable Command boundary; `M2-WF-003A` provides the bounded Queue delivery slice; `M2-WF-003B` provides only the private API-owned Claim/Heartbeat lease boundary. `M2-WF-003C — Lease and Delivery Reconciliation` is completed through PR #77, squash-merged as `ed428b1c12eb6e2ce01d964d56c05a09a3ba87d1`. `M2-SRC-003 — URL-capture Result Contract and Source Evidence Boundary` is completed through PR #82, squash-merged as `6b6e4a0f2c180093db6e76090ab14b831e5631f6`; it provides the API-owned versioned Fetcher Result contract, Task terminal states, idempotent/late-result semantics, immutable object integrity, and Source evidence handoff without public fetching. `M2-FETCH-001A — Public Transport and Resource Policy` is completed through PR #92, squash-merged as `551217c130f6717f4b8891ce76de1fa124bf8ee0` (`feat: add public URL transport policy (#92)`), with no runtime registration. `M2-FETCH-001B — Candidate Extraction and Scoped Snapshot Writer` is **In Review** with private unregistered Candidate/Snapshot preparation only; `M2-FETCH-001C` is Not Started, `M2-FETCH-001` remains incomplete, and real Fetcher URL execution remains unavailable. M3 remains not started.
...
| `M2-FETCH-001B — Candidate Extraction and Scoped Snapshot Writer`        | Extract the bounded Candidate, write and verify the immutable Snapshot, and construct the exact Result value.                                                                         | `M2-FETCH-001A` reviewed                                   | **In Review.** [Work Packet](work-packets/m2-fetch-001b-candidate-extraction-scoped-snapshot-writer.md), Issue #94. Private unregistered Candidate extraction, scoped Fetcher snapshot writer, and existing Result construction only. No Queue consumer, Task lifecycle orchestration, runtime registration, API, or Source mutation.                                                                                                                                                            |
| `M2-FETCH-001C — Queue-to-Gateway Fetcher Orchestration`                 | Consume the queued Task, coordinate Claim/Heartbeat/Result, and enforce one-active-capture process scheduling.                                                                        | `M2-FETCH-001A` and `M2-FETCH-001B` reviewed               | **Not Started.** No new transport, extraction, Source, or Workflow semantics. `M2-FETCH-001` remains incomplete until this bounded execution slice is accepted.                                                                                                                                                                                                                                                                                                                                  |
