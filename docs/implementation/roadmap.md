...
| `M2-QUAL-003 — Worker Dispatcher Reconciliation Observation Stability` | Wait for the authoritative Outbox acknowledgement after an initial or repaired Redis Job appears, without changing production behavior. | PR #146 final-head Integration smoke failure | **Blocked — final-v2 replay attributed a required concurrent failure to FG-07.** [Work Packet](work-packets/m2-qual-003-worker-dispatcher-observation-stability.md), Issue #147. The bounded initial/repaired-Job synchronization repair remains preserved and passed focused Worker 3× `7/7`, root `check` `53 files / 504 tests` plus builds, full Integration `27/184`, and Browser `16/16`. Required Concurrent attempts #1 and #2 passed; attempt #3 failed and stopped without rerun at `test=fetcher-gateway-api.test.ts case=fg-07`, with the remaining child clean and owned cleanup verified. No FG-07 root cause or repair is claimed. M2-QUAL-006 first closes a separately verified harness cleanup gap; a later focused Work Item must localize FG-07 before this repair is replayed again. M2-GOV-006 remains Blocked, M2 remains In Progress, and M3 remains Not Started. |
...
| `M2-QUAL-010 — Real FG-07 Evidence Replay` | Replay the real non-injected fixed FG-07 concurrent loop on the merged explicit child teardown evidence baseline. | M2-QUAL-009 completed | **Ready.** [Work Packet](work-packets/m2-qual-010-real-fg07-evidence-replay.md), Issue #169. Independent correctness/executability and governance/scope/security Definition of Ready reviews passed. The run-only diagnostic permits at most three sequential focused attempts and stops on the first red. A specific FG case or specific child cleanup classification defines a separate repair boundary; three passes are only `not reproduced`; generic or conflicting evidence, a non-clean remaining child, parent cleanup failure, or task-owned residue leaves the Work Item Blocked. No code, product, dependency, Schema, migration, Compose, CI, DEC, Acceptance Record, M2 completion, or M3 change. M2-QUAL-008, M2-QUAL-007, M2-QUAL-003, and M2-GOV-006 remain Blocked; M2 remains In Progress and M3 remains Not Started. |
