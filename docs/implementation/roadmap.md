...
| `M2-QUAL-006 — Managed Process Capture Cleanup Integrity` | Close the post-spawn/pre-publication ownership gap so an identity-capture failure cannot leave a task process while reporting clean teardown. | Verified task-owned orphan evidence during bounded M2 quality diagnosis | **Ready.** [Work Packet](work-packets/m2-qual-006-managed-process-capture-cleanup.md), Issue #157. Definition of Ready passed independent correctness and governance review. The repair is limited to immediate in-memory pending ownership, bounded rollback of the exact newly spawned process group, fail-closed cleanup evidence, focused tests, and harness documentation. No product behavior, global cleanup, PID-only recovery authority, new hash, dependency, Schema, migration, Compose, CI, DEC, acceptance, or M3 change. M2-QUAL-003 remains Blocked by the separately attributed FG-07 failure; M2-GOV-006 remains Blocked pending the required quality repairs; M2 remains In Progress and M3 remains Not Started. |
