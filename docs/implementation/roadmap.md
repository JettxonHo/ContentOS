...
**Current status: In Progress.** The M2 Source, Workflow, delivery, Gateway, recovery, Result, public-transport, Candidate/Snapshot, Queue-to-Gateway Fetcher, and current-Approved Source input projection foundations through `M2-SRC-004` are completed. `M2-FETCH-001` is **Completed** through PR #100, squash merge `4fe20a48a02b83ec68886bae68b86f5e65ba3895` (`feat: add queue-to-gateway Fetcher orchestration (#100)`); `M2-SRC-004` is **Completed** through PR #104, squash merge `d2ec063743681b64f0406b4903805700bd9866e3` (`feat: add approved Source input projection (#104)`). `M2-WF-004A` is implemented and **In Review** with authoritative read-only Workflow projection and Timeline REST queries. Later M2 work remains not started, M2 remains incomplete, and M3 remains not started.
...
| `M2-WF-004A — Workflow Projection and Timeline Query`                    | Add authenticated owner-scoped authoritative Workflow projection and bounded Timeline REST reads.                                                                                     | `M2-WF-003C`, `M2-SRC-004` completed                       | **In Review.** [Work Packet](work-packets/m2-wf-004a-workflow-projection-timeline-query.md), Issue #106. Core read Port, explicit PostgreSQL projection, exact Contracts, and two GET routes only. No SSE, polling timer, UI, write command, schema, migration, Queue, Agent, Research, or M3 behavior.                                                                                                                                                                                                                     |
