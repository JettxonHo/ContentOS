...
| `M2-WF-003B — Fetcher Gateway Claim and Bounded Lease`                   | Add the private API-owned Fetcher service Claim/Heartbeat contract and durable bounded Task lease.                                   | `M2-WF-003A`, `M2-DES-005` accepted | **Ready — not started.** Issue #71. API-owned Gateway, shared scoped service Secret, opaque claim hash, 60/20/120-second lease, and additive migration only. No Queue consumer, Fetcher public request, result, Source evidence, Object Storage, recovery, UI/SSE, Agent, or M3 behavior.                   |
