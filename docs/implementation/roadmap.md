...
| `M2-QUAL-003 — Worker Dispatcher Reconciliation Observation Stability` | Wait for the authoritative Outbox acknowledgement after an initial or repaired Redis Job appears, without changing production behavior. | PR #146 final-head Integration smoke failure | **Blocked — awaiting M2-QUAL-005.** [Work Packet](work-packets/m2-qual-003-worker-dispatcher-observation-stability.md), Issue #147. The bounded initial/repaired-Job synchronization repair passed focused, root, full Integration, and Browser evidence on `a72aecb`, but its first required concurrent run stopped at `test=fetcher-gateway-api.test.ts` with verified cleanup. The repair remains preserved but unaccepted and unpublished. M2 remains In Progress and M3 remains Not Started. |
...
| `M2-QUAL-005 — Safe Fetcher Gateway Case Attribution` | Refine the attributed Fetcher Gateway test-file failure to one static non-sensitive case ID without exposing raw output or changing test behavior. | M2-QUAL-004 completed; M2-QUAL-003 blocked at `fetcher-gateway-api.test.ts` | **In Review — independent review passed; awaiting GitHub CI.** [Work Packet](work-packets/m2-qual-005-safe-fetcher-gateway-case-attribution.md), Issue #153. The safe parser and `[FG-01]`–`[FG-11]` markers are implemented; focused evidence remains 31 tests. Two sandbox ordinary Integration setup attempts failed identically; two exact identity-confirmed task-owned orphan API processes from those attempts were targeted with `SIGTERM` and confirmed absent, while unrelated historical processes were untouched. Normal-permission Integration (27/184), root check (53 files/504 tests plus builds), Browser (16/16), and three sequential concurrent attempts all passed `RC=0`; concurrent produced no failure output. This records `not reproduced` only, with no root-cause or repair claim. M2-QUAL-003 and M2-GOV-006 remain Blocked; M2 remains In Progress and M3 remains Not Started. |
...
| `M2-GOV-006 — M2 Exit Review and Acceptance Record 002` | Re-run the complete M2 exit evidence on one exact post-remediation `main` commit and publish a new immutable Passed or Blocked record. | `M2-MAINT-002` completed; Record 001 retained | **Blocked pending M2-QUAL-003 and M2-QUAL-005.** [Work Packet](work-packets/m2-gov-006-m2-exit-review-002.md), Issue #144. Closed Draft PR #146 exposed a real integration-test observation race; the later concurrent run was safely attributed to `fetcher-gateway-api.test.ts` but not yet to a unique case or root cause. No Acceptance Record 002 was merged. M2 remains In Progress and M3 remains Not Started. |
