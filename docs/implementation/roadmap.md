...
| `M2-MAINT-002 — nanoid High Advisory Remediation` | Resolve the current vulnerable `nanoid@3.3.16` selection to the minimum patched `3.3.17` release without changing direct frameworks or product behavior. | M2 Acceptance Record 001 Blocked; Issue #139 open | **Ready.** [Work Packet](work-packets/m2-maint-002-nanoid-advisory-remediation.md), Issue #139. Exact transitive override and lockfile-only dependency repair; no product, API, Schema, migration, Compose, CI, Accepted DEC, or M3 behavior change. M2 remains In Progress; M3 remains Not Started. |
