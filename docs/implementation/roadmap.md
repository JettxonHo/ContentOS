...
| `M2-QUAL-003 — Worker Dispatcher Reconciliation Observation Stability` | Wait for the authoritative Outbox acknowledgement after a repaired Redis Job appears, without changing production behavior. | PR #146 final-head Integration smoke failure | **Ready.** [Work Packet](work-packets/m2-qual-003-worker-dispatcher-observation-stability.md), Issue #147. Two independent Definition of Ready reviews passed. Test synchronization only: preserve the exact `dispatched` assertion, add no arbitrary sleep, and make no production, dependency, Schema, migration, Compose, CI, DEC, or M3 change. M2 remains In Progress and M3 remains Not Started. |
...
| `M2-GOV-006 — M2 Exit Review and Acceptance Record 002` | Re-run the complete M2 exit evidence on one exact post-remediation `main` commit and publish a new immutable Passed or Blocked record. | `M2-MAINT-002` completed; Record 001 retained | **Blocked pending M2-QUAL-003.** [Work Packet](work-packets/m2-gov-006-m2-exit-review-002.md), Issue #144. Closed Draft PR #146 exposed a real integration-test observation race before publication; no Acceptance Record 002 was merged. After M2-QUAL-003 is completed, the exit review must restart from the new exact `main` commit. M2 remains In Progress and M3 remains Not Started. |
