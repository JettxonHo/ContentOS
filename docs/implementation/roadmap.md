...
| `M2-QUAL-017 — Correct-Tempdir Browser Recurrence and Artifact-Baseline Replay` | Establish the actual config-derived shared Playwright output baseline and run three predetermined full Browser recurrence slots without modifying code or publishing the Worker repair. | M2-QUAL-016 historical Blocked; Issues #200, #196, and #147 Open | **Ready.** [Work Packet](work-packets/m2-qual-017-correct-tempdir-browser-recurrence-artifact-baseline-replay.md), Issue #200. Independent DoR PASS by `/root/m2_qual_014_dor_correctness` and `/root/m2_qual_012_browser_setup_diagnosis`, role `DEFINITION_OF_READY_REVIEWER`, requested `gpt-5.6-sol` High, actual `UNVERIFIED_RUNTIME_MODEL`, reviewing base `001aaf6dbf1d2032c15b870c48838adefd4a3839` + exact-two Packet/Roadmap + Issue #200 parity with no findings. All phases are exact-two Packet+Roadmap. Fixed shared Browser output is observed from platform `tmpdir()` and never treated as task-owned residue or manually cleaned. Three sequential full Browser slots use normal permission from first invocation and first-red stopping. 3/3 green can become only Completed — Not Reproduced after independent review, final-head CI, and merge; it does not publish Worker or close #196/#147. QUAL016/003 remain Blocked, M2-GOV-006 remains Blocked, M2 remains In Progress, M3 remains Not Started. |
