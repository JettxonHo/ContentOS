...
| `M2-QUAL-009 — Explicit Child Teardown Record Emission` | Transport the existing sanitized Harness teardown record into the bounded child stderr capture before Vitest reports its generic close error. | M2-QUAL-008 blocked with parent cleanup verified and generic child cleanup evidence | **Ready.** [Work Packet](work-packets/m2-qual-009-explicit-child-teardown-record-emission.md), Issue #165. Definition of Ready passed independent correctness and governance review. Scope is one Harness-owned safe formatter, one LF-terminated global teardown emission before rethrow, focused tests, and exactly one synthetic injected live verification. It does not change cleanup, waits, retries, signals, ownership, capture, parser policy, product behavior, dependencies, CI, Acceptance Records, or M3. M2-QUAL-008, M2-QUAL-007, M2-QUAL-003, and M2-GOV-006 remain Blocked; M2 remains In Progress and M3 remains Not Started. |
