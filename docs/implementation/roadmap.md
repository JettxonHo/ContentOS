...
**Current status: In Progress.** `M2-SRC-001 — Pasted-text Source Capture and Approval Foundation` is completed (PR #40, `1a50d2d`), as is `M2-SRC-002 — .md/.txt File-upload Source Capture and Upload Quarantine` (PR #42, `3f418bf`), `M2-WF-001 — Template, Instance, Node, and Event Persistence Foundation` (PR #55, `8ec67ae`), and `M2-WF-002 — Atomic URL-capture Command, Task, and Transactional Outbox` (PR #60, `3928b55`). `M2-WF-003A — Transactional Outbox Dispatcher` is completed through PR #69, squash-merged as `3211c29ef8e6a934e6473a4f92caf36d8593abc3`. `M2-WF-002` adds only the protected durable Command boundary; `M2-WF-003A` provides the bounded Queue delivery slice; `M2-WF-003B` is now In Review with the private API-owned Claim/Heartbeat lease boundary. Fetcher URL execution, Source evidence, and later M2 Work Items remain unavailable; M3 remains not started.
...
| `M2-WF-003B — Fetcher Gateway Claim and Bounded Lease`                   | Add the private API-owned Fetcher service Claim/Heartbeat contract and durable bounded Task lease.                                   | `M2-WF-003A`, `M2-DES-005` accepted | **In Review.** Issue #71. Implemented with API/Core/Contracts/Database boundaries, additive `0007`, Fetcher configuration-validated skeleton, migration/concurrency/API/process/browser evidence, and no Queue consumer, Fetcher public request, result, Source evidence, Object Storage, recovery, UI/SSE, Agent, or M3 behavior. Awaiting independent review; not merged. |
