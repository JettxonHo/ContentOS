...
| `M2-QUAL-007 — FG-07 Focused Concurrent Localization` | Preserve real concurrent ownership while separating FG-07's Fetcher-failure and over-limit-body boundaries into a fixed diagnostic loop. | M2-QUAL-006 completed; M2-QUAL-003 blocked at `case=fg-07` | **Ready.** [Work Packet](work-packets/m2-qual-007-fg07-focused-concurrent-localization.md), Issue #161. Definition of Ready passed independent correctness and governance review. Scope is limited to splitting the composite test into `[FG-07A]` / `[FG-07B]`, a fixed non-injectable two-child focused command, safe attribution tests, and harness documentation. It does not change API behavior, body limits, timeout, retry, cleanup, dependencies, CI, Acceptance Records, or product capability. M2-QUAL-003 and M2-GOV-006 remain Blocked; M2 remains In Progress and M3 remains Not Started. |
