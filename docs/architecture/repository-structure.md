...
`core`, `contracts`, and `config` expose the bounded authentication, Content Package, Source, and Workflow values/Ports, including the framework-independent URL-capture Command and its versioned HTTP contracts, the private Fetcher Gateway Claim/Heartbeat contract, the internal Source-owned current-Approved input Port, the owner-scoped read-only Workflow Query Port/DTOs, the exact Workflow notification parser, and validated API, Fetcher, and Worker configuration. `database` owns the Drizzle Session, Content Package, Source, Workflow, URL Source Reference, Capture Request, Task lease, and Outbox schemas, node-postgres connection, transaction repository adapters, migration runner, and test-only persistence boundaries. Its Source input adapter implements the one current-Approved owner/Package projection without a Research caller or write capability; its Workflow Query adapter executes the package-rooted PostgreSQL projection and bounded Event page without a write capability. Its Worker-facing Dispatcher repository owns only transactional Outbox lease, acknowledgement, failure, missing-Job reconciliation, and the M2-WF-003C bounded expired-lease recovery transaction (completed, PR #77); its Fetcher Gateway repository owns only the API-owned PostgreSQL Claim/Heartbeat lease boundary; and its URL-capture Result repository (M2-SRC-003, completed through PR #82) owns the one-Result-per-Task terminal record, replay fencing, and the atomic Source-evidence/failure transaction. `object-storage` owns the S3-compatible ObjectStore adapter for Core's owner-scoped Source keys and the separate Fetcher-private public-URL snapshot writer introduced by M2-FETCH-001B; neither widens the other Port. `testing` owns deterministic unit/repository support and the isolated integration harness, including BullMQ delivery, lease-recovery, Fetcher Gateway, current-Approved Source projection, Workflow Timeline, and SSE notification assertions. Package build outputs are generated under ignored `dist/` directories.
