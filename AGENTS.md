...
- `corepack pnpm test:browser` runs the current owner-browser suite through Source, Research, Opinion/Research-based Mode, independently approved Blog/Xiaohongshu text, and all three downloads in pinned Playwright Chromium against the same isolated runtime boundary. Both Docker-dependent commands use `tmpfs`, ephemeral loopback ports, and temporary credentials outside the repository; both are intentionally excluded from `check`. Read [Integration Smoke Harness](docs/quality/integration-smoke-harness.md), [Browser Acceptance](docs/quality/browser-thin-slice.md), and [M2 Acceptance Harness](docs/quality/m2-acceptance-harness.md).
