...
**Current stage:** M1 completed; M2 is in progress (`M2-SRC-001` through `M2-SRC-004`, `M2-WF-001` through `M2-WF-004A`, and `M2-FETCH-001A` through `M2-FETCH-001C` are completed; `M2-FETCH-001` is completed; later M2 work remains not started).
...
The API owns Sessions, Content Package and Source state, the durable URL-capture request boundary, private Fetcher Gateway Claim/Heartbeat/Result, API-owned Source promotion, and the owner-scoped read-only `GET /v1/content-packages/:packageId/workflow` and `/workflow/events` queries. Those REST reads expose materialized Nodes and a bounded safe Event Timeline; PostgreSQL remains the Workflow truth. The Worker uses PostgreSQL and Redis to dispatch the fixed minimal BullMQ envelope and reconcile eligible expired leases. `M2-FETCH-001C` registers the Fetcher as the bounded consumer: it validates the current-generation Job, uses Fetcher-only Redis/Object Storage identities and controlled public transport, and submits Result through the Gateway. It never receives `DATABASE_URL` or writes Task, Workflow, or Source state directly. Web provides the M1 login, active/archived Dashboard, new-package form, and metadata/archive Workspace; later Source UI and Workflow stages are visibly unavailable. SSE is not implemented. The committed migrations create the M1 Source graph, Workflow catalog, URL-capture delivery boundary, Task leases, and Result boundary. No Agent, Render, publishing behavior, or deployment exists.
