...
**Current stage:** M0, M1, and M2 are completed. G1/M3 Research is effective; G2 Human Opinion and Blog is implemented by the current reviewed change.
...
The API owns Sessions, Content Package, Source, Research, Human Opinion, and Blog state; the durable URL-capture request boundary; private Fetcher Gateway Claim/Heartbeat/Result; API-owned Source promotion; owner-scoped Workflow reads; and notification-only SSE. Research and Blog generation persist restricted Raw Provider output but expose only validated structured content. PostgreSQL remains authoritative. The Worker dispatches the fixed minimal BullMQ envelope and reconciles eligible expired leases. The Fetcher uses only its scoped Redis/Object Storage/Gateway identities and never receives `DATABASE_URL`. Web provides login, Dashboard, metadata/archive Workspace, Source intake/review/Approval/Timeline, active-only Research review, and active-only Opinion/Blog generation, editing, checkpoint, exact Approval, and Markdown download. Archived review commands remain unavailable. The committed migrations create the Source/Workflow/Fetcher, Research, Opinion, and Blog boundaries. No real Provider, generic Agent runtime, Xiaohongshu, Render, rich Export, publishing, or deployment exists.
